"""Read-only access to the Dutch regular-verbs library (افعال منظم / Regelmatige Werkwoorden).

The data lives in the same remote PostgreSQL database as the B2 vocabulary
(``VOCAB_DATABASE_URL``), in schema ``vajegan-nl``, table
``Regelmatige_Werkwoorden``. Each row is one regular verb (e.g. ``werken``)
...
The table has no natural grouping column, so for browsing we group verbs by the
first letter of the infinitive (A, B, C …). This mirrors the particle grid of
the separable-verbs flow.
...
_TABLE = '"vajegan-nl"."Regelmatige_Werkwoorden"'
...
_VERB_COLUMNS = (
    ...
    "example_perfect_nl, example_perfect_fa"
)


@dataclass(frozen=True)
class RegularVerb:
    """A single regular verb read from the remote library."""
...
@dataclass(frozen=True)
class Letter:
    """A first-letter group and how many regular verbs start with it."""
...
def _to_verb(row: dict) -> RegularVerb:
    return RegularVerb(
        id=row["id"],
        rang=row["rang"],
...
async def get_verb_by_offset(letter: str, offset: int) -> RegularVerb | None:
